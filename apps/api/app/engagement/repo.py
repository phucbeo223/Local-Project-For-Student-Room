...
class EngagementRepository:
    ...
    def add_interaction(
        self, user_id: int, listing_id: int, interaction_type: str, duration_ms: int | None
    ) -> dict:
    ...
    def interaction_history(self, user_id: int, limit: int = 200) -> list[dict]:
        with self.engine.connect() as conn:
            rows = conn.execute(
                text(
                    "SELECT i.type, l.id, l.price, l.district, l.parsed_amenities "
                    "FROM user_interactions i JOIN aggregated_listings l ON l.id = i.listing_id "
                    "WHERE i.user_id = :user_id ORDER BY i.created_at DESC LIMIT :limit"
                ),
                {"user_id": user_id, "limit": limit},
            ).mappings().all()
        return [dict(row) for row in rows]

    def recommendation_candidates(self, limit: int = 250) -> list[dict]:
        with self.engine.connect() as conn:
            rows = conn.execute(
                text(
                    "SELECT id, price, district, parsed_amenities, quality_score, freshness_score, "
                    "risk_score, risk_evaluated_at FROM aggregated_listings "
                    "WHERE status = 'active' "
            ...
            for search in searches:
                ...
                for key, operator in (
                    ...
                    if value is not None:
                        column = "distance_to_ctu" if key == "max_distance_ctu" else key.replace("min_", "").replace("max_", "")
                ...
                for index, amenity in enumerate(criteria.get("amenities") or []):
                    ...
                    clauses.append(f"COALESCE((parsed_amenities ->> :{param})::boolean, false) IS TRUE")
                ...
                conn.execute(
                    text("UPDATE saved_searches SET last_notified_at = now(), updated_at = now() WHERE id = :id"),
                    {"id": search["id"]},
                )
        return created
