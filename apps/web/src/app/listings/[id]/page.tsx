...
const AMENITIES = [
  ...
  {
    label: "WC riêng",
    keywords: ["wc riêng", "vệ sinh riêng", "toilet riêng"],
  },
  {
    label: "Giờ giấc tự do",
    keywords: ["giờ tự do", "giờ giấc tự do", "không chung chủ"],
  },
  {
    label: "Cho nuôi thú cưng",
    keywords: ["nuôi thú cưng", "cho nuôi pet", "pet friendly"],
  },
];
...
function amenitiesFrom(listing: ListingOut): string[] {
  ...
  return Object.entries(listing.parsed_amenities || {})
      ...
      <main className="mx-auto max-w-[1240px] px-4 py-6 sm:px-8 lg:px-10 lg:py-8">
        ...
        >
          ...
          {listing.district && (
            <>
              <Link
                href={`/?district=${encodeURIComponent(listing.district)}`}
          ...
          <span className="max-w-[52ch] truncate font-medium text-ink">
            {listing.title}
          </span>
        </nav>
        ...
        <div className="mt-5 grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_350px]">
          ...
          <aside className="rounded-[22px] border border-line bg-white p-5 shadow-[0_12px_35px_-26px_rgba(6,48,92,.55)] lg:sticky lg:top-5 lg:p-6">
            ...
            <div className="mt-5 border-t border-line-soft pt-5">
              <p className="text-xs font-semibold uppercase tracking-[.08em] text-ink-faint">
                Đăng bởi
              </p>
              <div className="mt-3 flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-xs text-ink-muted">
                    {listing.source === "user"
                      ? "Tin do cộng đồng đăng"
                      : "Tin được hệ thống tổng hợp"}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="mt-6 grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_350px]">
          <article className="min-w-0 space-y-5">
            ...
            >
              <div className="flex flex-wrap gap-2">
                <span
                  className={`rounded-full px-3 py-1.5 text-xs font-semibold ${badge.className}`}
                >
                  {badge.label}
                </span>
              ...
              <nav
                className="mt-6 flex gap-1 overflow-x-auto border-b border-line-soft"
                aria-label="Các phần của tin"
              >
                {[
                  ["#tong-quan", "Tổng quan"],
                  ["#dac-diem", "Đặc điểm"],
                  ...
                  <Fact
                    ...
                    value={
                      listing.distance_to_ctu != null
                        ? formatDistance(listing.distance_to_ctu).replace(
                            "cách CTU ",
                            "",
                          )
                        : "Chưa rõ"
                ...
                {amenities.length > 0 && (
                  <div className="mt-5 flex flex-wrap gap-2">
                    {amenities.map((amenity) => (
                      <span
                        key={amenity}
                        className="rounded-xl border border-line bg-tint px-3 py-2 text-sm font-medium text-ink-soft"
                      >
                        ✓ {amenity}
                      </span>
                    ))}
            ...
            {routeTimes.length > 0 && (
              <section className="rounded-[20px] border border-line bg-white p-5 sm:p-7">
                <div className="flex flex-wrap items-end justify-between gap-2">
                  <h2 className="text-xl font-bold text-ink">
                    Thời gian đi xe máy tới trường
                  </h2>
                  <p className="text-xs text-ink-muted">
                    Tính theo dữ liệu đường thực tế
                  </p>
                </div>
                <div className="mt-4 grid gap-3 sm:grid-cols-3">
                  {routeTimes.slice(0, 3).map((minutes, index) => (
                    <div
                      key={campusNames[index]}
                      className="flex items-center justify-between rounded-2xl border border-line-soft bg-slate-50 p-4"
                    >
                      <div>
                      ...
                      <p className="text-2xl font-extrabold text-primary">
                        {Math.round(minutes)}
                        <span className="ml-1 text-xs font-medium">phút</span>
                      </p>
                    </div>
                  ))}
                </div>
            ...
            >
              ...
              {listing.description ? (
                <p className="mt-4 whitespace-pre-line text-[15.5px] leading-7 text-ink-soft">
                  {listing.description}
                </p>
              ) : (
                <p className="mt-4 text-sm text-ink-muted">
                  Người đăng chưa bổ sung mô tả cho tin này.
                </p>
              )}
            </section>
            ...
            {(listing.lat != null || listing.address) && (
              <section
                id="vi-tri"
                className="scroll-mt-4 overflow-hidden rounded-[20px] border border-line bg-white"
              >
                <div className="p-5 sm:p-7">
                  ...
                  <p className="mt-1 text-xs text-ink-muted">
                    Vị trí bản đồ có thể là vị trí tương đối; hãy xác nhận với
                    người đăng trước khi đi xem.
                  </p>
                </div>
            ...
            <section className="rounded-[20px] border border-amber-200 bg-amber-50 p-5 sm:p-6">
              <div className="flex gap-3">
                <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-amber-600 font-bold text-white">
                  !
                </span>
                <div>
                  <h2 className="font-bold text-amber-950">
                    Trước khi đặt cọc, hãy đi xem phòng
                  </h2>
                  <p className="mt-1.5 text-sm leading-6 text-amber-900">
                    Không chuyển tiền khi chưa gặp người cho thuê và chưa xem
                    hợp đồng. Kiểm tra kỹ giá điện, nước, tiền cọc và tình trạng
                    phòng thực tế.
                  </p>
                </div>
              </div>
            ...
            >
              ...
              {listing.report_count > 0 && (
                <p className="mt-4 rounded-xl bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700">
                  Tin này có {listing.report_count} phản hồi chưa bị bác bỏ từ
                  cộng đồng.
                </p>
              )}
              ...
              <section className="rounded-[20px] border border-line bg-white p-5 sm:p-6">
                ...
                <div className="mt-3 flex flex-wrap gap-3">
                  <Link
                    ...
                    className="rounded-xl border border-line px-4 py-2.5 text-sm font-semibold text-ink-soft hover:bg-tint"
                  >
                    Sửa thông tin
                  </Link>
          ...
          <aside className="hidden rounded-[20px] border border-line bg-white p-5 lg:block">
            ...
            <p className="mt-4 text-xs leading-5 text-ink-muted">
              Điểm số hỗ trợ sàng lọc và không thay thế việc xem phòng, xác minh
              người đăng.
            </p>
          </aside>
        </div>
          ...
          <section className="mt-7 rounded-[22px] border border-line bg-white p-5 sm:p-7">
            <div className="flex flex-wrap items-end justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-ink-muted">
                  {listing.district
                    ? `Các phòng khác tại ${listing.district}`
                    : "Các phòng mới cập nhật"}
                </p>
              </div>
              <Link
                href={
                  listing.district
                    ? `/?district=${encodeURIComponent(listing.district)}`
                    : "/"
                }
                className="text-sm font-semibold text-primary-bright hover:underline"
              >
                Xem tất cả →
              </Link>
            </div>
...
function Fact({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-line bg-slate-50 p-4">
      ...
      <p className="mt-1.5 break-words text-[17px] font-bold leading-snug text-ink">
        {value}
      </p>
    </div>
  );
